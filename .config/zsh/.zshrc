# Auto-start base tmux session (only outside tmux to prevent recursion)
if [[ -z "$TMUX" ]]; then
  ~/.config/tmux/base.sh
  exit
fi
...
# Prompt is rendered by starship (see end of file). Disable oh-my-zsh's theme.
ZSH_THEME=""
...
# Bare-repo dotfiles manager: tracks files in $HOME via the bare repo at $HOME/dotfiles
...
alias configurezsh="nvim ~/.config/zsh/.zshrc"
alias sourcezsh="source ~/.config/zsh/.zshrc"
alias configuretmux="nvim ~/.config/tmux/tmux.conf"
